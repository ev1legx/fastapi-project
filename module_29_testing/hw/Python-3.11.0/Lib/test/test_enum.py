...
class _EnumTests:
    ...
    def test_repr(self):
        ...
        if self.is_flag:
            ...
            self.assertEqual(
                repr(TE.dupe),
                "<MainEnum.third: %r>" % (self.values[2],),
                TE._value_repr_,
            )
        for name, value, member in zip(self.names, self.values, TE, strict=True):
            self.assertEqual(
                repr(member), "<MainEnum.%s: %r>" % (member.name, member.value)
            )
...
class _PlainOutputTests:
    ...
    def test_overridden_format(self):
        ...
        self.assertEqual(
            str(NF.first), "NewFormatEnum.first", "%s %r" % (NF.__str__, NF.first)
        )
...
class TestSpecial(unittest.TestCase):
    ...
    def test_extending(self):
        ...
        #
        with self.assertRaisesRegex(
            TypeError, "<enum .Foo.> cannot extend <enum .Color.>"
        ):
    ...
    @unittest.skip(
        "Accessing all values retained for performance reasons, see GH-93910"
    )
    def test_exception_for_member_from_member_access(self):
        with self.assertRaisesRegex(
            AttributeError, "<enum .Di.> member has no attribute .NO."
        ):
    ...
    def test_custom_flag_bitwise(self):
        ...
        self.assertTrue(
            isinstance(MyIntFlag.ONE | MyIntFlag.TWO, MyIntFlag),
...
class OldTestFlag(unittest.TestCase):
    ...
    def test_pickle(self):
        ...
        test_pickle_dump_load(
            self.assertEqual, IntFlagStooges.CURLY | IntFlagStooges.MOE
        )
        test_pickle_dump_load(
            self.assertEqual, IntFlagStooges.CURLY | IntFlagStooges.MOE | 0x30
        )
...
class TestVerify(unittest.TestCase):
    ...
    def test_continuous(self):
        ...
        #
        with self.assertRaisesRegex(
            ValueError, "invalid flag .Incomplete.: missing values 32"
        ):
...
class TestInternals(unittest.TestCase):
    ...
    def test_auto_garbage_ok(self):
        with self.assertWarnsRegex(
            DeprecationWarning, "will require all values to be sortable"
        ):
    ...
    def test_auto_garbage_fail(self):
        with self.assertRaisesRegex(
            TypeError, "will require all values to be sortable"
        ):
...
class TestStdLib(unittest.TestCase):
    ...
    def test_inspect_classify_class_attrs(self):
        ...
        values = [
            ...
            Attribute(
                name="__doc__", kind="data", defining_class=self.Color, object="..."
            ),
            Attribute(
                name="__getitem__",
                kind="method",
            ...
            Attribute(
                name="__name__", kind="data", defining_class=self.Color, object="Color"
            ),
            Attribute(
                name="__qualname__",
                kind="data",
            ...
            Attribute(
                ...
                kind="data",
                defining_class=self.Color,
                object=self.Color.YELLOW,
            ),
            Attribute(
                name="MAGENTA",
                kind="data",
                defining_class=self.Color,
                object=self.Color.MAGENTA,
            ),
            Attribute(
                name="CYAN",
                kind="data",
                defining_class=self.Color,
                object=self.Color.CYAN,
            ),
            Attribute(
                name="name",
                kind="data",
