...
class ReadTest(MixInCheckStateHandling):
    def check_partial(self, input, partialresults):
        ...
        result = ""
        for c, partialresult in zip(
            input.encode(self.encoding), partialresults, strict=True
        ):
            q.write(bytes([c]))
            result += r.read()
...
class UTF16Test(ReadTest, unittest.TestCase):
    ...
    def test_bug691291(self):
        ...
        with open(os_helper.TESTFN, "wb") as fp:
            fp.write(s)
        with codecs.open(os_helper.TESTFN, "r", encoding=self.encoding) as reader:
    ...
    def test_invalid_modes(self):
        ...
        for mode in ("rt", "wt", "at", "r+t"):
            with self.assertRaises(ValueError) as cm:
                codecs.open(os_helper.TESTFN, mode, encoding=self.encoding)
            self.assertIn("can't have text and binary mode at once", str(cm.exception))
