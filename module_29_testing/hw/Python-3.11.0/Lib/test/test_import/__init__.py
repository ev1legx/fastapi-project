...
class PycacheTests(unittest.TestCase):
    ...
    def test_missing_source_legacy(self):
        ...
        try:
            self.assertEqual(
                m.__file__, os.path.join(os.getcwd(), os.path.relpath(pyc_file))
            )
        finally:
            os.remove(pyc_file)
    ...
    def test___cached___legacy_pyc(self):
        ...
        m = __import__(TESTFN)
        self.assertEqual(
            m.__cached__, os.path.join(os.getcwd(), os.path.relpath(pyc_file))
        )
    ...
    def test_package___cached__(self):
        ...
        self.assertEqual(
            sys.modules["pep3147.foo"].__cached__, os.path.join(os.getcwd(), foo_pyc)
        )
