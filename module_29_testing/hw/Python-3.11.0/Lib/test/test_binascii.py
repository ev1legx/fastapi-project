...
class BinASCIITest(unittest.TestCase):
    ...
    def test_base64_strict_mode(self):
        ...
        def _assertRegexTemplate(
            assert_regex: str, data: bytes, non_strict_mode_expected_result: bytes
        ):
            with self.assertRaisesRegex(binascii.Error, assert_regex):
                binascii.a2b_base64(self.type2test(data), strict_mode=True)
            self.assertEqual(
                binascii.a2b_base64(self.type2test(data), strict_mode=False),
        ...
        def assertExcessData(data, non_strict_mode_expected_result: bytes):
            _assertRegexTemplate(
                r"(?i)Excess data", data, non_strict_mode_expected_result
            )
        ...
        def assertLeadingPadding(data, non_strict_mode_expected_result: bytes):
            _assertRegexTemplate(
                r"(?i)Leading padding", data, non_strict_mode_expected_result
            )
